...
write.csv(herb.site %>% filter(year==2019), file = 'sum_2019.csv')
```
...
```{r, echo=F, warning=F, message=F}
hfam.tran <- herb %>% 
  group_by(year, site, management, transect, family) %>% 
  summarize(bm_t = sum(biomass)/1.2, 
            den_t = n()/1.2) %>% 
...
hfam.l.site <- hfam.l.tran %>% 
  group_by(year, site, management, family) %>% 
  summarize(l_n = n(),
            l_s = mean(l_t), 
...
write.csv(hfam.site %>% filter(year==2019), file = 'sum_fam_2019.csv')
```
...
carn_fam <- c("Haemulidae","Lutjanidae","Serranidae") # focal families
